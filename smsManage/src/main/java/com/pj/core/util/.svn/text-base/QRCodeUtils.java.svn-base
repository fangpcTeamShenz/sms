...
public class QRCodeUtils {
	 ...
	 */
	public static void encodeQR(String contents, int width, int height,
			String imgPath) {
	...
	public static byte[] encodeQR(String contents, int width, int height) {
		Map<EncodeHintType, Object> hints = new HashMap<EncodeHintType, Object>();
		// 指定纠错等级
		hints.put(EncodeHintType.ERROR_CORRECTION, ErrorCorrectionLevel.L);
		// 指定编码格式
		...
		try {
			...
			return ImageUtil.image2Bytes(image, "png");
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	 ...
	 * @param destImagePath
	 */
	public static void encodeQR(String content, int width, int height,
		...
		try {
			...
			if (image == null) {
				throw new RuntimeException(
						"the decode image may be not exists.");
			}
	...
	public static void main(String[] args) {
		...
		String contents = "你好,李四! welcome to zxing!"
				...
				+ "\nEmail[xxx@163.com]";
		// 普通二维码的生成与解析
		QRCodeUtils.encodeQR(contents, 0, 10, imgPath);
		System.out.println("生成二维码成功");
		System.out.println(QRCodeUtils.decodeQR(imgPath));

		// 带图片的二维的生成与解析
		imgPath = "d:/pr2.png";
		String srcPath = "d:/src.jpg";
		QRCodeUtils.encodeQR(contents, 300, 300, srcPath, imgPath);
		System.out.println("生成带图片的二维码成功");
		System.out.println(QRCodeUtils.decodeQR(imgPath));

		// 条形码的生成与解析
		imgPath = "d:/bar.png";
		QRCodeUtils.encodeBar("6923450657713", 105, 50, imgPath);
		System.out.println("生成条形码成功");
		System.out.println(QRCodeUtils.decodeBar(imgPath));
	}
